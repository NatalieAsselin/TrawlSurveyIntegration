...
# ##load  datasets, if not re-running prep files
...
##save the final modeled dataset
write.csv(data, file = paste0(data.path, "/sGSL_SpatioTemporalModelDataset.csv"), row.names = FALSE)
...
a <- which(!is.na(data$commercial.weight))
...
##xlim and ylim are defined for this model in the <general values>
...
mesh <- make.barrier.mesh (data=data, xlim = xlim , ylim= ylim)
...
mod.select.fn <- function (){
  ...
  ##spatial model 
  c$AIC <- AIC (m)
  # c$cAIC <- cAIC (m) ##takes too long to run for all models. 
...
if ("m16" %in% models) {
 ...
 save(mod.select, cutoff, label, data, max.depth, min.depth,   years,   file = paste0(rdatapath, "sGSL_", label,  ".rdata"))
