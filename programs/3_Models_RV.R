...
##redefine xlim and ylim for this dataset
xlimRV = c(280, 720)
ylimRV = c(5055, 5432)
...
cutoff <- 15 ##from mesh selection results

mesh <- make.barrier.mesh (data=data, xlim = xlimRV , ylim= ylimRV)
...
if ("m4" %in% models) {
  ...
  m <- sdmTMB(  
    data=data,
    formula = commercial.weight ~  0 + as.factor(substrate) , 
    mesh = mesh,
    time_varying = ~ 1 ,
...
# ##spatial, time-varying depth.s effects
...
if ("m16" %in% models) {
 ...
 save(mod.select, cutoff, label, data, max.depth,  min.depth, years, xlimRV, ylimRV,  file = paste0(rdatapath, "sGSL_", label,  ".rdata"))
